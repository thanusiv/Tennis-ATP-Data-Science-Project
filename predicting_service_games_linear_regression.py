...
# Predicting percentage of service games won in a year based off the Service Game Columns
service_game_feature_columns = ['Aces', 'DoubleFaults', 'FirstServe', 'FirstServePointsWon', 'SecondServePointsWon', 'BreakPointsFaced', 'BreakPointsSaved', 'ServiceGamesPlayed', 'TotalServicePointsWon']
service_game_target_column = ['ServiceGamesWon']

all_combos = []
features_scores = {}

for t in range(1, len(service_game_feature_columns) + 1):
    combos_of_particular_size = [list(x) for x in combinations(service_game_feature_columns, t)]
...
print('Features that predicted the percentage of service games won by a player in a year')
...
plt.title('Actual Labels vs Model Predictions for Percentage of Service Games Won')
plt.xlabel('Percentage of Service Games Won (Labels)')
plt.ylabel('Percentage of Service Games Won (Model Predictions)')
